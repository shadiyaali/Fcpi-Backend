...
class PodcastSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if hosts_data:
            hosts = Speaker.objects.filter(id__in=[host['id'] for host in hosts_data])
            podcast.hosts.set(hosts)
        
        if guests_data:
            guests = Speaker.objects.filter(id__in=[guest['id'] for guest in guests_data])
...
def update(self, instance, validated_data):
    ...
    # Extract the hosts and guests from validated data
    hosts_data = validated_data.get('hosts', [])
    guests_data = validated_data.get('guests', [])
...
class PodcastUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Update ManyToMany relationships
        if hosts_data is not None:  # Use `is not None` to distinguish from an empty list
            instance.hosts.set(hosts_data)
