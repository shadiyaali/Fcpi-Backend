...
from .models import Admin,Forum,Speaker,Gallery,Attachment,GeneralEvent,GeneralAttachment,HostContentss,Podcasts,GuestContentss,Newsletter,GeneralCertificates,UserFileAssociation,GeneralMultiEvent,Event,GalleryImage,GeneralSingleEvent,SingleEvent,MultiEvent,Member,ForumMember,Blogs,BlogsContents,Certificates,Banner,News,BoardMember,Board,GeneralBlogsContents,GeneralBlogs
...
class GeneralMultiEventSerializer(serializers.ModelSerializer):
    speaker_name = serializers.SerializerMethodField()
...
class GeneralSingleEventSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        single_event = GeneralSingleEvent.objects.create(highlights=', '.join(highlights), **validated_data)
...
class GeneralSingleAllEventSerializer(serializers.ModelSerializer):
    event_name = serializers.CharField(source='event.event_name', read_only=True)
...
class NewsletterSerializer(serializers.ModelSerializer):
    class Meta:
        model = Newsletter
        fields = ['id', 'title', 'edition', 'start_date', 'end_date', 'thumbnail', 'pdf']
...
class  PodcastSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        podcast = Podcasts.objects.create(**validated_data)

        for content_data in host_contents_data:
            host_image_data = content_data.pop('host_image', None)
            host_content = HostContentss.objects.create(podcast=podcast, **content_data)
            if host_image_data:
    ...
    def create(self, validated_data):
        ...
        for content_data in guest_contents_data:
            ...
            guest_content = GuestContentss.objects.create(podcast=podcast, **content_data)
            if guest_image_data:
    ...
    def update(self, instance, validated_data):
        ...
        for content_data in host_contents_data:
            ...
            if content_id and content_id in existing_host_content_ids:
                content_instance = HostContentss.objects.get(id=content_id, podcast=instance)
                ...
                HostContentss.objects.create(podcast=instance, **content_data)

        for old_id in existing_host_content_ids:
            HostContentss.objects.get(id=old_id, podcast=instance).delete()
        ...
        for content_data in guest_contents_data:
            ...
            if content_id and content_id in existing_guest_content_ids:
                content_instance = GuestContentss.objects.get(id=content_id, podcast=instance)
                ...
                GuestContentss.objects.create(podcast=instance, **content_data)

        for old_id in existing_guest_content_ids:
            GuestContentss.objects.get(id=old_id, podcast=instance).delete()
